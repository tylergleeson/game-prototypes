...
//
// GENERATION of chained boards (pass 6) is `spec.sequence` / `spec.seqCost`, at the
// bottom of this file: the numbers are only earned if the board is measurably harder
// with them on than off, which is what stops a chain from being decoration.
...
// ---------- what the HINT BUTTON will cost on this board ----------
// `solve()` above answers "what is par" as cheaply as it can (one capped A*, plus a
// cascade fast path). The RUNTIME hint solver (`game.js solveFrom`) cannot: it is asked
// from an arbitrary live position and has no par to aim at, so it iterative-deepens —
// cap = remaining, remaining+1, ... remaining+extra — and every cap BELOW the answer is
// a complete miss that burns its whole state budget first. A board whose optimum sits
// three drags above the trivial bound therefore pays for two full failed searches
// before the third one answers, and the player feels that as a frozen tap.
//
// So it is measured here, at generation time, and a board that would make the hint
// button expensive is simply not shipped (`spec.maxHintStates`). This mirrors the
// runtime loop's SHAPE — same successor set, same admissible heuristic (h = blocks
// remaining), same per-cap budget — so the number it returns is the runtime's node
// count, not a guess; it is deliberately not the same code as `solve` because `solve`'s
// job is to be fast and this one's job is to be pessimistic in the same way the runtime
// is. Returns total states explored before the first solution (Infinity if none).
// `limit` makes rejection cheap: the generator only needs to know that a board is over
// budget, not by how much, so the walk stops the moment it passes the ceiling.
export function hintCost(level, { extra = 6, perCap = 80000, limit = Infinity, opts = {} } = {}) {
  const n = level.blocks.length;
  const chained = isChained(level);
  const start = level.blocks.map(b => [b.x, b.y]);
  let total = 0;
  for (let cap = n; cap <= n + extra; cap++) {
    const buckets = Array.from({ length: cap + 2 }, () => []);
    buckets[n].push([0, start]);
    const best = new Map([[stateKey(start), 0]]);
    let explored = 0, found = false;
    for (let f = n; f <= cap && !found;) {
      const bucket = buckets[f];
      if (!bucket.length) { f++; continue; }
      const [g, positions] = bucket.pop();
      const rem = positions.filter(Boolean).length;
      if (rem === 0) { found = true; break; }
      if (g + rem > cap) continue;
      if (++explored > perCap || total + explored > limit) break;
      const occ = makeOcc(level, positions);
      const nodeOpts = withRemaining(chained, opts, positions);
      for (let bi = 0; bi < n; bi++) {
        if (!positions[bi]) continue;
        const spots = reachable(level, occ, bi, positions[bi]);
        let exits = false;
        for (const [x, y] of spots) if (canExit(level, occ, bi, x, y, nodeOpts)) { exits = true; break; }
        const push = (np) => {
        ...
        if (exits) { const np = positions.slice(); np[bi] = null; push(np); }
        for (const [x, y] of spots) {
          if (x === positions[bi][0] && y === positions[bi][1]) continue;
          const np = positions.slice(); np[bi] = [x, y]; push(np);
        }
      }
    }
    total += explored;
...
export function genLevel(spec, opts = {}) {
  ...
  for (let attempt = 0; attempt < 400; attempt++) {
    ...
    if (!gatesOk) continue;
    // `spec.sequence` boards are graded by `fitChain` (solved twice, once bare and
    // once numbered) — everything below it is the unchained path, byte-for-byte the
    // code every shipped level before Sheet 4 was generated by.
    if (spec.sequence) {
      const chained = fitChain(level, spec, opts);
      if (chained) return chained;
      continue;
    }
    // `spec.maxStates` is undefined for every unchained spec (solver default
    // applies); the chained specs raise it per-spec without touching this.
...
// ---------- generating the approval chain (Sheet 4) ----------
// `spec.sequence = k` numbers k of the board's blocks 1..k; `spec.seqCost` is the
// minimum number of EXTRA drags those numbers must be worth.
//
// The board is solved twice — bare (`parFree`) and numbered (`parSeq`) — and the
// chain is only accepted if `parSeq - parFree >= seqCost`. That is the whole point:
// a permutation that happens to name the order a player would have taken anyway
// costs nothing and is decoration, so it is only allowed where the spec asks for it
// (`seqCost: 0`, the teaching level). `par` is `parSeq` — the par the player plays
// against — and the spec's excess band grades it.
//
// Permutations are retried before the board is rerolled: the same four blocks can be
// worth nothing in one order and three drags in another, and rerolling would throw
// away a board that was already good.

// deterministic shuffle on the module RNG, so a chained level is as reproducible
// from its seed as every unchained one
function shuffleR(a) { for (let i = a.length - 1; i > 0; i--) { const j = ri(i + 1); const t = a[i]; a[i] = a[j]; a[j] = t; } }

// k block indices to chain, PREFERRING DISTINCT COLOURS: colours are taken
// round-robin, so a 3-chain on a 4-colour board is three different colours and the
// numerals read as an order across the board rather than a grouping within one hue.
export function chainPick(level, k) {
  const byColor = new Map();
  level.blocks.forEach((b, i) => { if (!byColor.has(b.color)) byColor.set(b.color, []); byColor.get(b.color).push(i); });
  const groups = [...byColor.values()];
  for (const g of groups) shuffleR(g);
  shuffleR(groups);
  const out = [];
  for (let round = 0; out.length < k; round++) {
    ...
    for (const g of groups) {
      ...
      if (g[round] !== undefined) { out.push(g[round]); added = true; }
    }
    if (!added) break;
  }
...
function permsOf(k) {
  ...
  const rec = (cur, left) => {
    ...
    for (let i = 0; i < left.length; i++) rec(cur.concat(left[i]), left.filter((_, j) => j !== i));
  };
  rec([], Array.from({ length: k }, (_, i) => i + 1));
  return out;
}

// Returns the level with `blocks[i].seq` written and `par = parSeq`, or null (leaving
// the level exactly as it was found — no half-written chain ever escapes).
export function fitChain(level, spec, opts = {}) {
  const idx = chainPick(level, spec.sequence);
  if (!idx) return null;
  const cost = spec.seqCost || 0;
  const clear = () => { for (const bi of idx) delete level.blocks[bi].seq; };
  // the baseline: what this board costs with no numbers on it at all. Solved to the
  // same excess cap, so a board the chain would have to rescue is rejected here.
  const free = solve(level, spec.maxExcess, spec.maxStates, opts);
  if (!free || free.par < 0) return null;
  const perms = permsOf(spec.sequence);
  shuffleR(perms);
  const tries = Math.min(perms.length, spec.permTries || perms.length);
  for (let t = 0; t < tries; t++) {
    idx.forEach((bi, j) => { level.blocks[bi].seq = perms[t][j]; });
    const res = solve(level, spec.maxExcess, spec.maxStates, opts);
    clear();
    if (!res || res.par < 0) continue;
    const excess = res.par - level.blocks.length;
    if (excess < spec.minExcess || excess > spec.maxExcess) continue;
    if (res.par - free.par < cost) continue;
    idx.forEach((bi, j) => { level.blocks[bi].seq = perms[t][j]; });
    if (!meetsShape(level, spec, opts)) { clear(); continue; }
    // ...and the board must not make the hint button expensive. A chain that pushes the
    // optimum well above the trivial bound is exactly what the runtime's iterative
    // deepening pays for twice over, so the cost is measured and the board is dropped if
    // it is over budget — the hint is a shipped feature and a frozen tap is a broken one.
    if (spec.maxHintStates && hintCost(level, { limit: spec.maxHintStates, opts }) > spec.maxHintStates) { clear(); continue; }
    level.par = res.par;
    level.parFree = free.par; // generator-only bookkeeping; stripped before levels.js
    return level;
  }
  return null;
}
