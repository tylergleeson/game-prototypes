...
1. LOOK: `curl -s localhost:7411/state` → JSON: `mode` (confirms this is an adversarial session), `rules` (read once; the KEY RULE is one drag = one move, around corners), `buttons`, `screen` (menu | levels | legend | pause | win | fail | playing), `summary` (level, board, stones, blocks with id/color/origin/cells/escaped, gates, movesUsed/movesLeft/moveLimit/par, winCard/failCard text, rescueAvailable, `hud` {level, moves} text as displayed, `jsErrors` count + `recentErrors`), `budget` {minutesLeft, timeUp, levelsWon}, `lastResult`, `screenshot`. View the screenshot with the Read tool whenever the visual state matters (a mismatch between what the screen shows and what the JSON says IS a bug).
2. SAY (before every action): `curl -s -X POST localhost:7411/say -H 'content-type: application/json' -d '<json>'` with `{"say": "<what you are attacking / what you found, under 45 words>", "thought": "<one private sentence>", "note": null}` — or a bug: `"note": {"area": "bug|controls|ui|feedback|onboarding|difficulty|monetization|retention|legibility|other", "severity": "critical|major|minor|nit", "text": "REPRO: <exact steps incl. level and action JSON> · EXPECTED: <…> · ACTUAL: <…> · EVIDENCE: <turn/screenshot/error text>"}`. Severity: critical = crash, soft-lock, progress loss, JS error that breaks play; major = wrong game state, exploit (free moves, stars, rescue abuse), HUD lying about state; minor = visual/copy inconsistency, harmless error; nit = polish. Log every distinct bug once; log "held up" observations sparingly. Write awkward JSON to a file under /private/tmp and use `--data-binary @file`.
3. ACT: `curl -s -X POST localhost:7411/act -H 'content-type: application/json' -d '<json>'`, one of:
...
## Ending — hard rule
Check `budget.timeUp` on every /state. When true, stop acting and write your bug report as Markdown (under 1000 words) to <runDir>/review-draft.md with: 1) Summary — ship-blocking bugs count, overall robustness verdict, and a robustness score /10; 2) Bugs, ranked by severity, each with REPRO / EXPECTED / ACTUAL / EVIDENCE; 3) Exploits and inconsistencies found (or explicitly ruled out); 4) What held up under attack (specific); 5) Recommended regression checks for the playtest bot (one line each). Then file it:

node -e "const fs=require('fs');const r=fs.readFileSync('<runDir>/review-draft.md','utf8');fetch('http://localhost:7411/end',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({review:r})}).then(r=>r.json()).then(j=>console.log(JSON.stringify(j)))"
